...
def load_static_attributes(df: pd.DataFrame, attrs: List[str], basin_filename: str, meanstd_dir: Path, 
    ...
    if meanstd is None:
        ...
        mean = meanstd['mean']
        std = meanstd['std']
        logger.info('Using provided mean and std for normalizing static attributes')
...
def load_xarray_dataset(dataset_path: Path, basins: List[str], 
    ...
    if meanstd is None:
        ...
        logger.info('Using provided mean and std for normalizing dynamic data')
...
class MyDataset(Dataset):
    def __init__(self, 
                 ...
                 hindcast_length: int = 8,    # 336h = 14d, 后顾时间的长度
    ...
    def __getitem__(self, idx: int):
        ...
        x_d = hindcast_sample[self.forcing_attrs]
        x_s = self.static_ds[basin]
        y = forecast_sample[self.target_vars]

        # 转换成torch.Tensor
        x_d = x_d.to_array(dim='variable').transpose('date', 'variable')
        x_d = torch.from_numpy(x_d.values).float()

        y = y.to_array(dim='variable').transpose('date', 'variable')
        y = torch.from_numpy(y.values.squeeze()).float()

        return {'x_d': x_d, 'x_s': x_s, 'y': y}
